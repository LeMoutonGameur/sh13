...
int main(int argc, char *argv[])
{
        	...
        	switch (buffer[0])
        	{
				...
				// Si le nombre de joueurs atteint 4, alors on peut lancer le jeu
                                ...
                                if (nbClients==4){
									...
									// Envoi des cartes et infos à tous les joueurs 
									for (int i = 0; i < 4; i++) {
									    // Format: "D card1 card2 card3 table0 table1 ... table7"
									    sprintf(reply, "D %d %d %d %d %d %d %d %d %d %d %d %d", 
									        deck[i*3], deck[i*3+1], deck[i*3+2],   // 3 cartes du joueur
									        tableCartes[i][0], tableCartes[i][1],   // Ligne complète de tableCartes
									        tableCartes[i][2], tableCartes[i][3],
									        tableCartes[i][4], tableCartes[i][5],
									        tableCartes[i][6], tableCartes[i][7],
									        12);                                   // coupable

									    sendMessageToClient(tcpClients[i].ipAddress, tcpClients[i].port, reply);
									}

									// Définition du joueur courant (0 = premier joueur)
									joueurCourant = 0;
									char reply[256];
									sprintf(reply, "M %d", joueurCourant);
									broadcastMessage(reply);  // Envoi à tous les joueurs
	...
	else if (fsmServer==1)
	{
		switch (buffer[0])
		{
			case 'G': // Accusation (ex: "G 1 12" = joueur 1 accuse Moriarty)
			    int accusedCard;
			    sscanf(buffer, "G %*d %d", &accusedCard); // %*d pour ignorer l'ID
			    if (accusedCard == deck[12]) { // Comparer avec la carte coupable
			        sprintf(reply, "W %s", tcpClients[joueurCourant].name);
			        broadcastMessage(reply); // Annonce du gagnant
			    } else {
			        joueurCourant = (joueurCourant + 1) % 4; // Passer au joueur suivant
			        sprintf(reply, "M %d", joueurCourant);
			        broadcastMessage(reply);
			    }
			    break;
			...
			case 'O': {
			    ...
			    for (int i = 0; i < 4; i++) {
			        if (i == demandeur) continue; // On ne répond pas au joueur qui a posé la question

			        const char* reponse = (tableCartes[i][objet] > 0) ? "oui" : "non";
			        sprintf(reply, "V %d %d %s", i, objet, reponse);
			...
			case 'S': // Demande à un joueur (ex: "S 1 2 3" = joueur 1 demande l'objet 3 au joueur 2)
			    int targetPlayer, objet;
			    sscanf(buffer, "S %*d %d %d", &targetPlayer, &objet);
			    sprintf(reply, "V %d %d %d", targetPlayer, objet, tableCartes[targetPlayer][objet]);
			    broadcastMessage(reply);
			    joueurCourant = (joueurCourant + 1) % 4;
			    sprintf(reply, "M %d", joueurCourant);
			    broadcastMessage(reply);
