...
int guiltGuess[13];
int tableCartes[4][8];
int isQualitative[4][8] = {{0}}; // 1 = oui/non, 0 = quantité exacte
int b[3];
int goEnabled;
int connectEnabled;
   ...
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);
	...
	if (SDL_PollEvent(&event))
	{
        ...
        if (synchro==1)
        {
			...
			case 'I': // Réception de l'ID
			    	sscanf(gbuffer, "I %d", &gId);
			    	break;

			case 'L': // Liste des joueurs
			        sscanf(gbuffer, "L %s %s %s %s", gNames[0], gNames[1], gNames[2], gNames[3]);

    				// Affichage console pour vérifier que ça marche
    				printf("Liste des joueurs reçue :\n");
    				for (int i = 0; i < 4; i++) {
        				printf("Joueur %d : %s\n", i, gNames[i]);
					}
			    	break;

			case 'D': // Cartes reçues (3 cartes + tableCartes)
			    sscanf(gbuffer, "D %d %d %d %d %d %d %d %d %d %d %d %d", 
			        &b[0], &b[1], &b[2],
			        &tableCartes[gId][0], &tableCartes[gId][1], &tableCartes[gId][2],
			        &tableCartes[gId][3], &tableCartes[gId][4], &tableCartes[gId][5],
			        &tableCartes[gId][6], &tableCartes[gId][7], &guiltSel);
			    break;

			case 'M': // Joueur courant
			    int currentPlayer;
			    sscanf(gbuffer, "M %d", &currentPlayer);
			    goEnabled = (currentPlayer == gId); // Active le bouton si c'est notre tour
			    break;

			case 'V': {
			    int player, objet;
			    char reponse[16];
			    sscanf(gbuffer, "V %d %d %s", &player, &objet, reponse);

			    if (strcmp(reponse, "oui") == 0) { // j'ai / j'ai pas
			        tableCartes[player][objet] = 1;
			        isQualitative[player][objet] = 1;
			    } else if (strcmp(reponse, "non") == 0) {
			        tableCartes[player][objet] = 0;
			        isQualitative[player][objet] = 1;
			    } else {
			        tableCartes[player][objet] = atoi(reponse); // quantite
			        isQualitative[player][objet] = 0;
			    }

			    // Debug console
			    if (isQualitative[player][objet])
			        printf("→ Le joueur %d : objet %d => %s\n", player, objet, reponse);
			    else
			        printf("→ Le joueur %d : objet %d => possède %d exemplaire(s)\n", player, objet, tableCartes[player][objet]);
			    break;
			}
		    ...
		    for (j = 0; j < 8; j++) {
		        if (tableCartes[i][j] != -1) {
		            ...
		            if (tableCartes[i][j] == 100) {
		                sprintf(mess, "*");
		            } else if (isQualitative[i][j]) {
		                sprintf(mess, "%s", tableCartes[i][j] ? "oui" : "non");
		            } else {
	...
	if (goEnabled==1)
	{
    		...
    		SDL_Rect boutonGo = {500, 350, 200, 100};
    		SDL_RenderFillRect(renderer, &boutonGo);

    		SDL_RenderCopy(renderer, texture_gobutton, NULL, &boutonGo);
	}
	// Le bouton connect
	if (connectEnabled==1)
	{
